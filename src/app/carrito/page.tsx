      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[--color-text-muted]">
            {totalCount === 0 ? "No hay productos" : `${totalCount} ${totalCount === 1 ? "producto" : "productos"}`}
          </p>
        </div>
        {items.length > 0 && (
          <button
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              <div className="rounded-2xl border border-[--color-border-subtle] bg-[--color-card] p-6">
                ...
                <div className="space-y-2 border-b border-[--color-border-subtle] pb-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-[--color-text-muted]">Productos ({totalCount})</span>
                    <span className="text-[--color-text-primary]">{currency.format(total)}</span>
                  </div>
                </div>
                <div className="mt-4 flex justify-between text-lg font-bold">
                  <span className="text-[--color-text-primary]">Total</span>
                  <span className="text-[--color-accent-secondary]">{currency.format(total)}</span>
                </div>
              </div>

              <div className="rounded-2xl border border-[--color-border-subtle] bg-[--color-card] p-4">
                <label htmlFor="order-note" className="mb-2 block text-sm font-medium text-[--color-text-primary]">
                ...
                <textarea
                  ...
                  placeholder="Ej: Necesito envío express, regalo con mensaje especial, etc."
                  rows={3}
                  className="w-full rounded-lg border border-[--color-border-subtle] bg-[--color-panel] px-3 py-2 text-sm text-[--color-text-primary] placeholder:text-[--color-text-muted] focus:border-[--color-accent-primary] focus:outline-none focus:ring-2 focus:ring-[--color-accent-primary]/30"
                />
              </div>
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] px-6 py-3 font-semibold text-white transition hover:bg-[#20BA5A] disabled:cursor-not-allowed disabled:opacity-50"
              >
